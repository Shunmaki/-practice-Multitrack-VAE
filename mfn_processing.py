"""Code taken from Music FaderNets https://github.com/gudgud96/music-fader-nets
"""
...
CLASSIC_PIANO_PATH = "/home3/adam/codes/music-fader-nets/scripts/classicpiano/"
PIANO_ECOMPETITION_PATH = "/home3/adam/codes/music-fader-nets/scripts/epianocomp/"
...
def process_data(name, beat_res=4, num_of_beats=4, max_tokens=100, multitrack=False):
    '''
    Utility function for each data function to extract required data.
    Set multitrack to True to process MIDI to multitrack MIDI-like representation.
    '''
    data_lst = []
    rhythm_lst = []
    note_density_lst = []
    ...
    if len(track) > 0:
        ...
        # TODO
        # if multitrack:
        #     pr = [tr.pianoroll for tr in track]
        # else:
            ...
            if end_idx // beat_res < len(beats):
                ...
                if len(new_pm.instruments[0].notes) > 0 and \
                    ...
                    if rhythm[0] == 1 and len(events) <= max_tokens:   
                        chroma = get_harmony_vector("tmp.mid")    # read from saved "tmp.mid" file
...
def get_classic_piano(data_type="short", path="/home3/adam/codes/music-fader-nets/data/values_v3/", dry_run=False):
    '''
    Main data function for Yamaha Piano e-Competition dataset.
    '''
    dataset_path = pathlib.Path(path)
    labelled_midi = [CLASSIC_PIANO_PATH + k \
                      for k in os.listdir(CLASSIC_PIANO_PATH)]
    labelled_midi += [PIANO_ECOMPETITION_PATH + k \
                      for k in os.listdir(PIANO_ECOMPETITION_PATH)]

    print("Dataset length:", len(labelled_midi))
    keylst = labelled_midi

    if not (dataset_path / "data.npy").exists() or dry_run:
        data_lst = []
        rhythm_lst = []
        note_density_lst = []
        tempo_change_lst = []
        ...
        if not dry_run:
            np.save(dataset_path / "data.npy", data_lst)
            np.save(dataset_path / "rhythm.npy", rhythm_lst)
            np.save(dataset_path / "note_density.npy", note_density_lst)
            np.save(dataset_path / "chroma.npy", chroma_lst)
            print("Dataset saved!")
    
    else:
        data_lst = np.load(dataset_path / "data.npy")
        rhythm_lst = np.load(dataset_path / "rhythm.npy")
        note_density_lst = np.load(dataset_path / "note_density.npy")
        chroma_lst = np.load(dataset_path / "chroma.npy", allow_pickle=True)

        # sanitization
        idx = []

        for i in tqdm(range(len(chroma_lst)), desc="Sanitization"):
            try:
                c = chroma_lst[i]
